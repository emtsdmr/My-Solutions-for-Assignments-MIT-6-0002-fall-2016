...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        posnew=pos.get_new_position(self.direction, self.speed)#important
        if self.room.is_position_valid(posnew):
            self.set_robot_position(posnew)
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.gets_faulty():
            ...
            pos=self.get_robot_position()
            posnew=pos.get_new_position(self.direction, self.speed)
            if self.room.is_position_valid(posnew):
                self.set_robot_position(posnew)
                self.room.clean_tile_at_position(self.position, self.capacity)
            else:
...
def step(num_robots,roomClass,robots,min_cov):
    ticks=0
    while float(roomClass.get_num_cleaned_tiles()/roomClass.get_num_tiles())<min_cov:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        ticks.append(step(num_robots,room,robots,min_coverage))
    
    return sum(ticks)/num_trials
